...
@Component
public class TokenRootInterceptor implements HandlerInterceptor {
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       ...
       OnlyRoot onlyRoot = ((HandlerMethod) handler).getMethodAnnotation(OnlyRoot.class);
        if(onlyRoot == null){
            //放行
            return  true;
        ...
        System.out.println(token);
         ...
         //不是root用户返回状态码10012
        if(!TokenUtils.root(token)){


            response.getWriter().println(10012);
            return  false;
        }
